...
final weightProvider =
    StateNotifierProvider<WeightNotifier, AsyncValue<List<WeightLog>>>((ref) {
      return WeightNotifier(ref.read(weightRepositoryProvider));
    });

class WeightNotifier extends StateNotifier<AsyncValue<List<WeightLog>>> {
  final WeightRepository repository;

  WeightNotifier(this.repository) : super(const AsyncLoading()) {
    loadWeights();
  }

  Future<void> loadWeights() async {
    try {
      final weights = await repository.getAllWeights();
  ...
  Future<void> addWeight(WeightLog weight) async {
  ...
  Future<void> deleteAllWeights() async {
    await repository.deleteAllWeights();

    await loadWeights();
  }

  Future<void> updateWeight(WeightLog weight) async {
